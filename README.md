...
- 系统会按每个标样批次在 Excel 中实际存在的位点进行对比，`测定位点数` 为每个批次下唯一位点数。
- 同一批次同一位点若存在多条样本记录，会全部参与对比，并按位点合并输出，不会重复计数。
- 同一位点出现多条记录且判定冲突时，按最坏结果归类：`缺失 > 差异 > 不完全相同 > 相同`。
- `缺失位点数/缺失位置` 按样本缺失口径统计：样本无该位点，或样本 `A/B/C` 全为 `0`。
